...
The device can be selected by index or name substring via `PROXY_AUDIO_INPUT_DEVICE`. If the configured sample rate isn't supported, AudioIO falls back to the device's default rate and propagates it to all downstream consumers.
